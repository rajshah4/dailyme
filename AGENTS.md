...
- **Reddit 403 in cloud/dev environments** — Reddit blocks JSON API requests from data-center IPs with 403. Fixed with Reddit OAuth2: set `REDDIT_CLIENT_ID` + `REDDIT_CLIENT_SECRET` (create a "script" app at https://www.reddit.com/prefs/apps). When credentials are present, `_get_reddit_oauth_token()` fetches a client_credentials token and requests route to `oauth.reddit.com` (bypasses IP blocks). Without credentials the pipeline falls back gracefully (skips Reddit with a warning). Add both vars to GitHub repo secrets AND OpenHands Cloud repo secrets so the automation picks them up.
